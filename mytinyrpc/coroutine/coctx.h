 ...
 *
 * 此文件不感知栈布局，只定义寄存器在 coctx::regs[] 数组中的位置，
 * 偏移值由 coctx_swap.S 的保存/恢复顺序决定，两者必须同步。
 */
...
enum {
    ...
    kRETAddr = 9,  // 返回地址，coctx_swap 会跳转到此处继续执行
    kRSP = 13,     // rsp — 栈顶指针
};

// coctx 保存 14 个通用寄存器的值，用于协程上下文切换。
// 顺序必须与 coctx_swap.S 中的保存/恢复顺序严格一致。
struct coctx {
    void* regs[14];
};
 ...
 */
extern void coctx_swap(tinyrpc::coctx* old_ctx, tinyrpc::coctx* new_ctx);
