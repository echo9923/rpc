...
coctxSwap:
    ...
    # kRSP = 13 → 偏移 104
    # %rsp: 栈顶指针寄存器（Stack Pointer），指向当前栈的顶部
    # %rax: 通用寄存器（Accumulator），用作临时存储
    leaq    (%rsp), %rax          # leaq: 加载有效地址。把 %rsp（栈顶指针）的值复制到 %rax
