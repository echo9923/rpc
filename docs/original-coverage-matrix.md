...
| `generator` | `generator/tinyrpc_generator.py`、`generator/template/*` | 已复刻核心工程语义 | 111、112、113、114、115、124 | CLI、simple/full layout、`protoc` 生成 `.pb.h/.pb.cc` 和 descriptor-set、descriptor service/method 解析、多 service 选择、method interface、service 适配、business exception、test client、生成工程 CMake、启动/调用/关闭脚本，生成 server 收到 SIGTERM/SIGINT 后调用 `StopRpcServer()`。 | `./scripts/check_generator.sh`、`./scripts/check_generator_project.sh` |
...
| 发布级独立工程打包 | 保留简化 | full layout 生成原项目风格目录，但仍依赖本地 MyTinyRPC 源码路径；阶段 26 已补框架层 server stop，尚未做安装包和脱离源码树的发布形态。 | 需要发布形态时单独补安装和打包能力。 |
...
| 需要新计划 | HTTP keep-alive/chunked/streaming、更完整服务发现/多目标 RPC、OpenTelemetry/跨进程 trace、发布级生成工程打包。 | `TcpServer::stop()` 已在阶段 26 补齐；其余能力未来如需要应单独建新阶段。 |
...
当前项目已经覆盖 TinyRPC 学习主线中的配置、日志、启动入口、运行时、Reactor、Timer、TCP、TinyPB、HTTP、协程、异步 RPC 和生成器。阶段 18 到阶段 25 已经把早期简化项集中补齐并完成收口；阶段 26 已补齐框架层 `TcpServer::stop()` 和服务端生命周期专项验收。剩余边界属于明确保留的生产级扩展能力，不是阶段 25/26 的遗漏。
