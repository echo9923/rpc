...
当前阶段仍以学习网络 IO 行为为主，不追求并发性能。任务十八完成后，`TcpConnection` 的基本形态是：输入缓冲区 + 输出缓冲区 + Reactor 读写事件。
...
- 定义最小编解码接口，让协议层从输入缓冲区判断是否存在完整消息。
- 当前 Echo 行为可先通过最简单的透传编解码保持不变。
- 不急于引入 HTTP 或 TinyPB 的具体协议格式。
- 不在任务十九中引入线程、协程、Timer 或任务队列。
