...
| 层级 | 承载位置 | 含义 | 示例 |
|---|---|---|---|
| Controller error | `TinyPbRpcController::ErrorCode()` / `ErrorText()` | 客户端框架层失败，调用方不应继续信任业务 response | connect 失败、timeout、msgReq mismatch、response 反序列化失败 |
...
| `ERROR_RPC_MSGREQ_MISMATCH` | 100013 | Controller | response `msgReq` 与 request `msgReq` 不一致 | `TinyPbRpcChannelTest.MismatchedResponseMsgReqSetsControllerError` |
