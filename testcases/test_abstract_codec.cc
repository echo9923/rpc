...
// 继承 AbstractData，添加 payload 字段用于存储编解码的字符串内容。
// ─────────────────────────────────────────────────────────────────────────────
...
// encode：将 StringData::payload 追加到 TcpBuffer。
// decode：从 TcpBuffer 取出全部可读数据写回 StringData::payload。
// ─────────────────────────────────────────────────────────────────────────────
...
{
    StringData data;
    // 新构造的 AbstractData，encodeSucc 和 decodeSucc 均应为 false
    EXPECT_FALSE(data.encodeSucc);
    EXPECT_FALSE(data.decodeSucc);
}

// ─────────────────────────────────────────────────────────────────────────────
// Test 2：StringCodec::encode 将 payload 写入 TcpBuffer 并设置 encodeSucc
// ─────────────────────────────────────────────────────────────────────────────
TEST(AbstractCodecTest, StringCodecEncode)
{
    StringCodec codec;
    StringData data;
    data.payload = "hello codec";
    ...
    // encode 后 encodeSucc 应为 true
    EXPECT_TRUE(data.encodeSucc);

    // buffer 中应有 "hello codec" 共 11 字节
    EXPECT_EQ(buffer.getReadableBytes(), data.payload.size());
    EXPECT_EQ(buffer.retrieveAllAsString(), "hello codec");
}

// ─────────────────────────────────────────────────────────────────────────────
// Test 3：StringCodec::decode 从 TcpBuffer 读取数据到 payload 并设置 decodeSucc
// ─────────────────────────────────────────────────────────────────────────────
TEST(AbstractCodecTest, StringCodecDecode)
{
    ...
    // decode 后 decodeSucc 应为 true，payload 应为 "decode test"
    EXPECT_TRUE(data.decodeSucc);
    EXPECT_EQ(data.payload, "decode test");
